...
// Script to generate a sprite sheet of all the icons in the svg-icons directory
// From Jacob Paris: https://www.jacobparis.com/content/svg-icons
...
const namesContent = await generateNames({
  names: files.map((file) => JSON.stringify(file.replace(/\.svg$/, ''))),
})

await writeIfChanged(path.join(outputDir, 'names.ts'), namesContent)

async function generateNames({ names }: { names: string[] }) {
  return [
